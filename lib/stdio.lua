...
local function start_repl()
  ...
  local function _2_(val)
    return io_channel:push(val)
  end
  out = _2_
  local options = nil
  local function _3_(kind, ...)
    return out({kind, "Error:", ...})
  end
  options = {moduleName = "fennel", onError = _3_, onValues = out, pp = view, readChunk = coroutine.yield}
  coroutine.resume(coro, options)
  thread:start("eval", io_channel)
  local function _4_(input)
    return coroutine.resume(coro, (input .. "\n"))
  end
  love.handlers.eval = _4_
  return nil
end
